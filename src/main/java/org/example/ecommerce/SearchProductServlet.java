...
public class SearchProductServlet extends HttpServlet {
    ...
    private void updateProduct(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            String query = "UPDATE product SET product_name=?, product_description=?, product_price=?, product_quantity=?, category_id=?, image_path=? WHERE product_id=?";
            ...
            if (rowsAffected > 0) {
                resp.sendRedirect("updateProduct.jsp?message=Product updated successfully");
            } else {
                resp.sendRedirect("updateProduct.jsp?error=Product not updated");
            }

            connection.close();
        } catch (Exception e) {
            e.printStackTrace();
            resp.sendRedirect("updateProduct.jsp?error=An error occurred while updating the product");
        }
    }
    ...
    private void deleteProduct(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            if (rowsAffected > 0) {
                resp.sendRedirect("updateProduct.jsp?message=Product deleted successfully");
            } else {
                resp.sendRedirect("updateProduct.jsp?error=Product not deleted");
            }

            connection.close();
        } catch (Exception e) {
            e.printStackTrace();
            resp.sendRedirect("updateProduct.jsp?error=An error occurred while deleting the product");
        }
    }
